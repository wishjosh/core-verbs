...
`MAKE` 단원의 100문장은 `data/make-chunk-overrides.json`에서 별도로 직접 검수합니다. 각 문장에는 첫 학습용 `microChunks`·`microOrderGlosses`와 숙달 뒤 사용할 `assemblyChunks`·`orderGlosses`가 함께 들어 있습니다. 짧은 단계에서도 관사+명사, 부정·조동사+동사, 구동사와 사역 구문처럼 함께 익혀야 하는 결합은 임의로 끊지 않습니다. 영어 원문은 두 단계 모두에서 글자와 문장부호까지 동일하게 복원되어야 합니다.
