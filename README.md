...
5. 앱은 문법 유형을 미리 단정하지 않고, 선택한 실제 단어 위치와 이어진 구간, 어순 오류와 문장 전체 회상 실패 여부를 문장과 함께 기록합니다.
6. `단어 오류 없음` 또는 `선택한 오류 저장`으로 자기 채점을 마치면 결과가 복습 일정에 반영됩니다. 오답 원자료는 최근 100건까지만 별도로 보관하며 설정에서 JSON 파일로 내보낸 뒤 외부 AI에 직접 첨부해 반복 패턴을 분석할 수 있습니다.
